...
import type { LogicalRect } from '../utils/coordinates.js';
import type { TextBlock } from '../utils/text-grouping.js';
import { normalizeForComparison } from './recent-outputs.js';

export interface DedupOptions {
  /** How long an entry suppresses matching text, in ms. Default 3000. */
  readonly windowMs?: number;
  /** Index cell size in logical px. Default 12. Affects performance, not verdicts. */
  readonly gridSize?: number;
  /**
   * How far a box may move and still be the same box, in logical px, on each axis
   * independently. Default 12 - comfortably above the +-3px jitter design doc section 5
   * describes, and below a line height, so the line below is never mistaken for this one.
   */
  readonly positionTolerance?: number;
  /** Minimum similarity for a fuzzy match, 0..1. Default 0.95; see the module comment. */
  readonly similarityThreshold?: number;
  /**
   * Shortest normalized string that may be compared fuzzily. Default 8. Below it, only exact
   * equality counts - a one-character difference in a short string is a large fraction of it,
   * and `yes`/`yep` are not the same message.
   */
  readonly fuzzyMinLength?: number;
  /** Shortest normalized candidate the prefix rule will accept. Default 6. */
  readonly prefixMinLength?: number;
  /** Fraction of the seen string a prefix candidate must cover. Default 0.5. */
  readonly prefixMinRatio?: number;
  /**
   * Hard cap on retained entries. Default 256. The time window normally bounds the set; this
   * bounds it too when a caller passes a clock that does not advance.
   */
...
/** Which rule matched. Carried into the decision so a log line can name it. */
export type DedupReason = 'exact' | 'prefix' | 'similar';
...
export type DedupDecision =
  ...
  | {
      ...
      /** The normalized text already on record that this candidate matched. */
      readonly matchedText: string;
      /** 1 for `exact`; the covered fraction for `prefix`; the similarity for `similar`. */
...
export class Deduplicator {
  ...
  readonly #cells = new Map<string, DedupEntry[]>();
  ...
  get size(): number {
    let total = 0;
    for (const entries of this.#cells.values()) total += entries.length;
    return total;
  }
   ...
   */
  admit(candidate: DedupCandidate, nowMs: number): DedupDecision {
    this.#expire(nowMs);
    ...
    if (text.length === 0) return { duplicate: false };
  ...
  *#nearbyEntries(x: number, y: number): Generator<DedupEntry> {
  ...
  #expire(nowMs: number): void {
    ...
    for (const [key, entries] of this.#cells) {
      ...
      else if (kept.length !== entries.length) this.#cells.set(key, kept);
    }
  }
   ...
   */
  #trim(): void {
    const live: DedupEntry[] = [];
    for (const entries of this.#cells.values()) live.push(...entries);
...
export interface DedupeBlocksResult {
 ...
 */
function compare(candidate: NormalizedCandidate, entry: DedupEntry, options: ResolvedOptions): Match | undefined {
  if (candidate.text === entry.text) return { reason: 'exact', similarity: 1 };

  // A one-character difference in a five-character string is not jitter, it is a different word.
  if (Math.min(candidate.text.length, entry.text.length) < options.fuzzyMinLength) return undefined;

  // The number changed, so the text changed. No similarity score gets to overrule this.
  if (candidate.digits !== entry.digits) return undefined;
  ...
  if (score >= options.similarityThreshold) return { reason: 'similar', similarity: score };

 ...
 */
import { unionRects } from './coordinates.js';
 ...
 */
export function groupLines(lines, options = {}) {
...
function breaksBlock(previous, next, paragraphGapRatio, columnGapRatio) {
    ...
    if (horizontalGap(previous.rect, next.rect) / referenceHeight > columnGapRatio)
        return true;
...
function buildBlock(lines) {
    ...
    if (bbox === undefined)
        throw new Error('cannot build a text block from zero lines');
    return {
        lines: [...lines],

...
**early exit ของ `ChangeDetector` ประหยัดน้อยกว่าที่ชื่อมันบอกมาก** — วัดจริง: อ่าน sample **น้อยลง 50 เท่า** (686 จาก 34,286) แต่เร็วขึ้นแค่ **1.5 เท่า** (0.0305ms เทียบ 0.0459ms) แปลว่า**ต้นทุนต่อ sample ไม่ใช่ต้นทุนหลัก** — มีค่าคงที่ครอบอยู่ที่กินเวลาเกือบทั้งหมด · ทั้งสองตัวเลขอยู่ในระดับ noise ของ budget 15ms อยู่แล้ว **อย่าไปปรับจูน diff loop เพื่อหวังกำไรด้าน latency** กำไรมันไม่ได้อยู่ตรงนั้น ([#70](https://github.com/zsitthiporn/textlens/issues/70) ตั้งคำถามว่ากลไกนี้คุ้มกับความซับซ้อนไหม)
...
- ~~เครื่อง dev ยังไม่มี .NET 10~~ → **เครื่องนี้มี .NET 10 SDK แล้ว (10.0.111/303/400) sidecar target `net10.0-windows10.0.19041.0`** ตามที่ design doc §2 ตั้งใจไว้แต่แรก · NativeAOT + WinRT publish ผ่านแล้วจริง ได้ exe เดี่ยว **4.72MB** (เลข 1.64MB ที่เคยเขียนไว้มาจาก build วันที่ 02:58 ซึ่ง**เก่ากว่า protocol ที่ landed 03:30** — ดู PKG-01)
...
ข้อควรรู้เรื่อง crash ของ main process — **วัดบน Electron 43.4.0 จริง ไม่ใช่อ่าน docs** ([#67](https://github.com/zsitthiporn/textlens/issues/67)):
- **Electron override นโยบาย unhandled rejection ของ Node กลับเป็น `warn`** — Node 24 default เป็น `throw` แต่ Electron 43 ไม่ใช่ **process จึงรอดชีวิต** วัดได้จริง
  → แปลว่า `void somePromise.then(...)` ที่ reject **ไม่ได้ทำให้แอปตาย มันทำให้แอปเดินต่อโดยข้าม `.then()` ไป** ซึ่งเงียบกว่าการตายมาก
  → กรณีจริงที่เจอ: `shutdown()` reject → `app.quit()` ไม่ถูกเรียก แต่ `tray.destroy()` รันไปแล้วและ `preventDefault()` ยกเลิก quit ถาวร = **process ที่ยังอยู่โดยไม่มีทางออกที่ผู้ใช้กดได้** เหลือแค่ Task Manager
  → **อย่าเขียนว่า "ขึ้นกับ default ของ Node"** — Node ไม่ได้เป็นคนตัดสินเรื่องนี้
- **uncaught exception ที่ไม่มี handler → error box มาตรฐานของ Electron บล็อก message loop เกิน 20 วินาที** timer ทุกตัวแข็งหมดระหว่างนั้น
  → เป็นแหล่ง blocking ของ main process ที่ทำให้ test ที่ไวต่อเวลาดูเหมือน flaky ได้ ถ้าบังเอิญมี dialog เปิดอยู่
...
| เรื่อง | ต้องรู้ |
|---|---|
| **NativeAOT publish ล้มใน agent shell** | `NoDefaultCurrentDirectoryInExePath=1` ถูก set ที่ process scope (ไม่ใช่ User/Machine ไม่ใช่ profile — มาจาก process chain ของ terminal) มันทำให้ `VsDevCmd.bat` หา `vswhere.exe` ไม่เจอ แล้ว error text ถูก MSBuild splice เข้าไปใน `$(CppLinker)` โผล่เป็น `MSB3073 ... exited with code 123` ซึ่งไม่บอกอะไรเลย **แก้: เคลียร์ตัวแปรก่อน publish** หรือใส่ `C:\Program Files (x86)\Microsoft Visual Studio\Installer` ลง PATH — ไม่ใช่บั๊กของ csproj · **ตอนนี้ `scripts/build-sidecar.ps1` จัดการให้แล้ว** ไม่ต้องจำเป็นเงื่อนไขก่อนรันอีก |
| **⚠️ อย่าใช้ sidecar exe ที่ build ค้างไว้** | `bin/Release/.../publish/` มี exe เก่าค้างได้ และมัน**พูดคนละ protocol** กับ Node โดยไม่มีอะไรฟ้อง — เคยเจอจริง: exe build 02:58 แต่ protocol landed 03:30, OCR/WGC 04:03, change detection 05:32 · ถ้าเอาไป package จะได้แอปที่เปิดขึ้น หา sidecar เจอ แล้ว**ไม่มี frame มาเลย** ซึ่งใน log หน้าตาเกือบเหมือนสำเร็จ · `scripts/build-sidecar.ps1` จึง**บังคับ rebuild เสมอ ไม่มีทาง reuse** |
...
| **⚠️ ห้าม assert ขนาดไฟล์ log ทันทีหลังเรียก log** | `sonic-boom` เรียก `_actualWrite()` **ใน stack ของผู้เรียก** แต่ยิง **async `fs.write`** เข้า libuv threadpool (`writeSync` ใช้เฉพาะ sync mode) → ไบต์ถึงดิสก์ก่อนบรรทัดถัดไปทำงานได้ โดยที่**ไม่มีอะไร block เลย** · วัดจริง: 150 trial ใต้ 8 spinning thread → **2 trial เห็นข้อความ 66 byte ครบบนดิสก์แล้ว ขณะที่ sync syscall เป็น 0 ทั้ง 150** · test ที่ assert `statSync(...).size === 0` จึงล้ม ~1 ใน 5 full-suite run โดยไม่ได้วัดสิ่งที่มันอ้าง ([#63](https://github.com/zsitthiporn/textlens/issues/63)) · **ถ้าอยากรู้ว่า log block ไหม ให้ census syscall ไม่ใช่แข่งกับมัน** และต้องมี positive control ไม่งั้น assertion เชิงลบผ่านแบบว่างเปล่าได้ |
...
| **⚠️ `windows-latest` เปิดโปง timing bug ที่เครื่องนี้ไม่มีวันเห็น** | CI รอบแรกสุดของโปรเจกต์ ([#68](https://github.com/zsitthiporn/textlens/issues/68)) จับได้ทันที 2 เรื่อง · `CaptureLoopTests` ล้ม **2 ใน 2 รอบ** บน runner แต่**ผ่านบนเครื่องนี้ทุกครั้ง** ([#72](https://github.com/zsitthiporn/textlens/issues/72)) และรอบสองมี test ตัวที่สองใน class เดียวกันล้มเพิ่ม · ส่วน [#71](https://github.com/zsitthiporn/textlens/issues/71) ล้ม **1 ใน 10 ที่นี่ แต่ 1 ใน 2 บน runner** · **runner เป็น VM core น้อยที่แชร์กับคนอื่น ซึ่งใกล้เคียงสภาพเครื่องผู้ใช้ตอนเล่นเกม fullscreen มากกว่าเครื่อง dev ที่ว่าง** → "ผ่านบนเครื่องผมแล้ว" ไม่ใช่หลักฐานสำหรับอะไรที่พึ่งจังหวะเวลา |
| **⚠️ agent หลายตัวใน clone เดียวกันมองเห็นการแก้ของกันและกัน** | worker ตัวหนึ่งอ่าน README แล้วรายงานว่า issue เขียนผิด (บอกว่า `>= 20` แต่ไฟล์เขียน `>= 22.12.0`) — ความจริงคือ**อีก worker เพิ่งแก้ไปเมื่อสิบนาทีก่อนใน session เดียวกัน** issue ถูกต้องตอนที่เปิด · **เวลาจะยืนยันว่า issue ยัง valid ไหม ต้องอ่านผ่าน baseline commit** (`git show <base>:<path>`) ไม่ใช่ working tree · เป็นเหตุผลเดียวกับที่ต้อง fetch ก่อนอ่าน ไม่ใช่เชื่อ checkout |
| **⚠️ NUL byte ในไฟล์ source ทำให้ ripgrep/grep ข้ามไฟล์นั้นทั้งไฟล์แบบเงียบ** | เจอจริง 2 ไฟล์ที่เขียน `join('<NUL ตัวจริง>')` แทนที่จะเขียน escape `'\0'` — **grep และ ripgrep จัดว่าเป็น binary แล้วข้าม ไม่มี warning** ผลคือ `renderSignature` ที่นิยามใน `transitions.ts` ค้นเจอเฉพาะที่ `layout.ts` ซึ่งเรียกใช้ ราวกับไม่มีนิยาม · **การ rename ทั้งโปรเจกต์จะพลาดไฟล์พวกนี้เงียบๆ** และ `Select-String` ของ PowerShell **หาเจอ** จึงยิ่งหลอกว่าไม่มีปัญหา · เขียน `'\0'` เสมอ ให้ผลเหมือนกันทุกประการ · ตรวจทั้ง repo: `node -e "..."` อ่านไฟล์แล้วนับ `\0` — อย่าใช้ grep หาสิ่งที่ grep มองไม่เห็น |

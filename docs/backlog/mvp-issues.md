...
<!-- ISSUE -->
title: M3-03 Noise filter — length, size, patterns
labels: electron, pipeline
depends: M3-02
...
> ⚠️ **ไม่มีเกณฑ์ confidence** — `Windows.Media.Ocr` ไม่ส่งค่านี้มาเลย ยืนยันด้วย reflection (2026-08-16): `OcrWord` มีแค่ `Text` กับ `BoundingRect` `conf` บน wire เป็น optional และจะไม่ถูกส่งมาเมื่อใช้ engine นี้
> **ห้ามเดา heuristic มาแทน** — เป็นความผิดพลาดชนิดเดียวกับการ "แก้" `o`↔`O` ที่ spike S1 สรุปว่าอย่าทำ
> เหตุผลที่เกณฑ์ที่เหลือยังพอ: S1 พบว่าความผิดพลาดที่ OCR ทำประจำ **ไม่กระทบความหมาย** → ขยะที่ต้องกรองจริงคือ*บรรทัดขยะทั้งบรรทัด* ไม่ใช่บรรทัดที่อ่านเพี้ยนนิดหน่อย ซึ่งจับได้ด้วย length/bbox/pattern อยู่แล้ว
> ถ้าใช้จริงแล้วกรองไม่พอ ค่อยกลับมาหา proxy signal โดยมีข้อมูลจริงประกอบ (ดู #47)
...
- เรียงความสำคัญด้วย **area** (ข้อความใหญ่มาก่อน)
- เกินโควตา → ตัดตัวท้ายทิ้ง
- เรียงลำดับการวางจากบนลงล่างเพื่อให้ผลคงที่

> ⚠️ **ไม่มี `× confidence`** — `Windows.Media.Ocr` ไม่ส่ง confidence มาเลย (ดู M3-03 และ #47)
> area อย่างเดียวเป็น proxy ที่ใช้ได้จริงกับ use case หลัก เพราะ subtitle มักเป็นข้อความใหญ่สุดในกรอบอยู่แล้ว
> **ห้ามประดิษฐ์ค่า confidence ขึ้นมาเพื่อให้สูตรเดิมยังใช้ได้**

## Acceptance criteria

- [ ] พื้นที่รวมไม่เกินสัดส่วนที่ตั้งไว้
- [ ] ข้อความใหญ่ถูกเลือกก่อนข้อความเล็ก
